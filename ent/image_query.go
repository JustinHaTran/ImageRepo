...
func (iq *ImageQuery) QueryOwner() *UserQuery {
...
// QueryImagerepos chains the current query on the imagerepos edge.
func (iq *ImageQuery) QueryImagerepos() *ImageRepoQuery {
	query := &ImageRepoQuery{config: iq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// First returns the first Image entity in the query. Returns *NotFoundError when no image was found.
...
// FirstID returns the first Image id in the query. Returns *NotFoundError when no id was found.
...
// Only returns the only Image entity in the query, returns an error if not exactly one entity was returned.
...
// OnlyID returns the only Image id in the query, returns an error if not exactly one id was returned.
...
func (iq *ImageQuery) Clone() *ImageQuery {
	...
	return &ImageQuery{
		...
		withImagerepos: iq.withImagerepos.Clone(),
...
func (iq *ImageQuery) WithOwner(opts ...func(*UserQuery)) *ImageQuery {
	query := &UserQuery{config: iq.config}
...
//  WithImagerepos tells the query-builder to eager-loads the nodes that are connected to
// the "imagerepos" edge. The optional arguments used to configure the query builder of the edge.
func (iq *ImageQuery) WithImagerepos(opts ...func(*ImageRepoQuery)) *ImageQuery {
	query := &ImageRepoQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
	iq.withImagerepos = query
	return iq
}
...
func (iq *ImageQuery) Select(field string, fields ...string) *ImageSelect {
	selector := &ImageSelect{config: iq.config}
...
func (iq *ImageQuery) sqlAll(ctx context.Context) ([]*Image, error) {
	...
	if query := iq.withOwner; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].user_images; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_images" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := iq.withImagerepos; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Image, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Imagerepos = []*ImageRepo{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(image.ImagereposPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, iq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "imagerepos": %v`, err)
		}
		query.Where(imagerepo.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "imagerepos" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Imagerepos = append(nodes[i].Edges.Imagerepos, n)
			}
		}
	}
...
func (iq *ImageQuery) sqlQuery() *sql.Selector {
...
// ImageGroupBy is the builder for group-by Image entities.
...
// ImageSelect is the builder for select fields of Image entities.
