...
type ImageRepoFunc func(context.Context, *ent.ImageRepoMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ImageRepoFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ImageRepoMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ImageRepoMutation", m)
	}
	return f(ctx, mv)
}
